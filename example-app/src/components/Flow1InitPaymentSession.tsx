...
import type { InitPaymentSession, PaymentSessionHandler } from '@juspay-tech/capacitor-hyperswitch';
import type { HyperswitchSession } from '@juspay-tech/capacitor-hyperswitch';
import { HyperElements, CvcWidget } from '@juspay-tech/capacitor-react-hyperswitch';
...
export default function Flow1InitPaymentSession({ fetchedData, elementsSession, elementsToken }: Props) {
  ...
  const initPaymentSession = async () => {
    if (!fetchedData) { setInitStatus('Fetch first.'); return; }
    setInitStatus('Initializing…');
    try {
      const hyper = Hyperswitch.init({ publishableKey: fetchedData.publishableKey });
      const s = await hyper.initPaymentSession({ sdkAuthorization: fetchedData.sdkAuthorization });
      setSession(s);
  ...
  const presentSheet = async () => {
    if (!session) { setSheetOutput('Call initPaymentSession first.'); return; }
    setSheetOutput('Presenting…');
    try {
      const result = await session.presentPaymentSheet();
      setSheetOutput(resultText(result));
    } catch (err: unknown) {
      setSheetOutput('Error: ' + (err instanceof Error ? err.message : String(err)));
    }
  };
  ...
  const getCustomerSavedPaymentMethods = async () => {
    if (!session) { setSavedMethodsOutput('Call initPaymentSession first.'); return; }
    ...
    try {
      ...
      setSavedMethodsOutput('Error: ' + (err instanceof Error ? err.message : String(err)));
    }
  };

  const getLastUsedMethod = async () => {
    if (!handler) { setSavedDataOutput('Call getCustomerSavedPaymentMethods first.'); return; }
    try {
      const data = await handler.getCustomerLastUsedPaymentMethodData();
      setSavedDataOutput('Last used:\n' + JSON.stringify(data, null, 2));
    } catch (err: unknown) {
      setSavedDataOutput('Error: ' + (err instanceof Error ? err.message : String(err)));
    }
  };
  ...
  const confirmWithLastUsed = async () => {
    if (!handler) { setConfirmSavedOutput('Call getCustomerSavedPaymentMethods first.'); return; }
    ...
    try {
      ...
      setConfirmSavedOutput('Error: ' + (err instanceof Error ? err.message : String(err)));
    }
  };
  ...
  return (
    <section>
      ...
      {elementsSession && elementsToken ? (
        <HyperElements hyper={elementsSession} options={{ sdkAuthorization: elementsToken }}>
          {cvcMounted && (
            <CvcWidget
        ...
        <div className="cvc-placeholder">CVCWidget (requires Elements session — call elements() in Flow 2 first)</div>
      )}
      ...
      <div className="btn-row" style={{ marginTop: 8 }}>
        <button onClick={getLastUsedMethod} disabled={!handler}>Get Last Used</button>
        <button onClick={getDefaultMethod} disabled={!handler}>Get Default</button>
      </div>
      <pre>{savedDataOutput}</pre>

      <div className="btn-row" style={{ marginTop: 8 }}>
        <button onClick={confirmWithLastUsed} disabled={!handler}>Confirm Last Used + CVC</button>
        <button onClick={confirmWithDefault} disabled={!handler}>Confirm Default + CVC</button>
      </div>
